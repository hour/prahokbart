...
ls $datadir/train.summary.$src* | egrep 'data/train.summary\..+\.[0-9]+' | xargs rm -f
ls $datadir/train.title.$trg* | egrep 'data/train.title\..+\.[0-9]+' | xargs rm -f
...
$py $yanmtt/train_nmt.py -n 1 -nr 0 -g 1 -p $PORT $args \
    ...
    --train_src $datadir/train.summary.$src --train_tgt $datadir/train.title.$trg \
    --dev_src $datadir/val.summary.$src --dev_tgt $datadir/val.title.$trg \
    ...
    --dev_batch_size 64 \
    --max_src_length 64 --max_tgt_length 32 \
